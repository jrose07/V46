...
Im ersten Teil der Auswertung wird das maximal B-Feld bestimmt. Hierbei sind die Messwerte zur Bestimmung des B-Feldes in Abhängigkeit des Abstands aufgetragen.

\begin{table}[H]
    \centering
    \caption{Messwerte des B-Feldes entlang der z-Achse}
...
Die Eigenschaften der einzelnen Proben sind in folgender Tabelle dargestellt. Hierbei ist $N$ die Dotierungskonzentration, $d$ die Dicke der Probe und $n$~\cite{Brechungsindex} der Brechungsindex der Probe.
\begin{table}[H]
    \centering
    ...
    \begin{tabular}{lccc}
        \toprule
        & \textbf{Probe 1} & \textbf{Probe 2} & \textbf{Probe 3} \\
        \midrule
        $N$ [cm$^{-3}$] & $2,8 \cdot 10^{18}$ & $1,2 \cdot 10^{18}$ & $0$ \\
        $d$ [mm] & $1,296$ & $1,36$ & $5,11$ \\
        $n$ & $3,3543$ & $3,3543$ & $3,3543$ \\
        \bottomrule
    \end{tabular}
\end{table}

Im nächsten Schritt werden die Messwerte der Drehwinkel der einzelnen Proben bei verschieden Wellenlängen der Interferenzfilter aufgetragen.
...
Mithilfe von 
\begin{equation}
    1 rad = \frac{\qty{360}{\degree}}{2 \pi}\end{equation}
und 
\begin{equation*}
    \qty{1}{\arcminute} = \frac{1}{60} \unit{\degree}\end{equation*}
wird der Winkel vom Gradmaß in Bogenmaß umgerechnet.
Der Winkel $\theta$ wird mithilfe von 
\begin{equation*}
    \theta_{skaliert} = \frac{\theta_1 - \theta_2}{2L}
\end{equation*}
skaliert.
...
\begin{figure}[H]
    ...
    \caption{Drehwinkel der ersten n-dotierten Probe in Abhängigkeit der Wellenlänge.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{content/plots/theta_vs_lambda_n2.png}
    \caption{Drehwinkel der zweiten n-dotierten Probe in Abhängigkeit der Wellenlänge.}
\end{figure}
\subsection{Bestimmung der effektiven Masse}
Im Folgenden wird die Differenz der Drehwinkel der n-dotierten Proben zu der undotierten Probe gebildet, um die effektive Masse zu bestimmen.
